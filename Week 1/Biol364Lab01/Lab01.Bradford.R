...
# Load in the tidyverse library, which contains many packages that we will use throughout the semester

if (!require("tidyverse")) install.packages("tidyverse"); library(tidyverse)

# Note the syntax above first checks to see if the package has already been installed, installs it if it hasn't, and then loads the library
...
# ggplot2 was already loaded in as part of the Tidyverse
...
# with a few tweaks. First lets make sure the graph shows the full range of possible data down to 0 concentration
...
# Now lets apply a theme to simplify the appearance of the graph
...
# and the confidence interval is large. With more datapoints, hopefully the results would be better.
