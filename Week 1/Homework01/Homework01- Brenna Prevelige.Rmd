...
summary(Supplementary_material)
```
The Supplementary material dataset has 16 variables. Right now, I am interested in the brain mass, body mass, and hibernation.
...
```{r}
qqnorm(normalizedbrain/normalizedbody, main='Normal Q-Q Plot for Log Transformed Relative Brain Size')
...
```
I used aesthetics that I learned from online supplementary materials: http://t-redactyl.io/blog/2016/04/creating-plots-in-r-using-ggplot2-part-10-boxplots.htm. This looks like much more normal distribution
```{r}
...
```
Above is the transformed relative brain mass distribution. It looks less normalized, as confirmed by the Q-Q plot above. However, I want to use relative brain mass in this analysis since it is what the authors used in their hypothesis.
...
ggplot(Supplementary_material, aes(x=as.factor(hibernation), y=(log(`brain mass`/`body mass`,10)), group=hibernation)) + 
  geom_boxplot() + scale_x_discrete(name = "Hibernation") + scale_y_continuous(name = "Log(Brain Mass/Body Mass)") + ggtitle("Boxplot of Log(Relative Brain Mass) by Hibernation") + theme_bw() + 
              theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
```
...
```
The log transformed brain over the log transformed body was statistically significant with a p value of 2.419e-08. This is saying that with 95% confidence, we can say that the mean relative brain size for non-hibernating animals is significantly different than the mean relative brain size for hibernating animals. This confirms the authors' hypothesis.
However, I say this with little confidence. The data were not normally distributed, especially with the relative brain mass that was log transformed. The authors take into account phylogenetics when coming to their conclusion, which we cannot do here. Therefore, while my analysis deems it unlikely that their hypothesis is confirmed, deeper statistical analysis using phylogenetic relationships likely will.

I wanted to test the hypothesis that presence of hibernation behaviors are correlated with substrate use. Mammals that are living in areas that are consistent year round or can manipulate environments to keep them relatively constant (like fossorial mammals that can burrow in the ground) likely don't need to hibernate. However, I would expect that animals in more traditional terrestrial environments that change seasonally are more likely to hibernate due to shifting resource availability. I wanted to see the makeup of hibernating animals below based on substrate use. This essentially is the same (less clear) version of the graph coming after. But I liked the color, so it stays. 
...
ggplot(Supplementary_material) +
  ...
  geom_bar() +
  theme_cowplot()+ scale_x_discrete(name = "Hibernation") + scale_y_continuous(name = "Count")+ ggtitle("Substrate Use Representation in Hibernation") 
```

Below is a clearer visual showing which animals hibernate based on their substrate use. A 0 indicates no hibernating behavior, with a 1 indicating hibernating behavior.
```{r}
...
This graph indicates that the majority of hibernating animals use substrates 3 and 5. Substrate 3 is terrestrial or semi-arboreal, and substrate 5 is volant. There were also some arboreal hibernators. As predicted, mammals in more dynamic environments, like changing terrestrial environments, have higher counts of hibernation. Mammals in more stable environments, such as fossorial (substrate 2) have no hibernation behaviors. 
...
```
With a p value of 2.144e-12, I can reject the null hypothesis. There is a signficant relationship between substrate use and hibernation. However, it is important to note that this chi-square approximation may be incorrect. I cannot just assume that this p-value is correct and that substrate use may have affected the evolution of hibernation. This may be due to errors in calculation with R, the dataset, or again something like confounding variables such as phylogenies affecting the data. 
...
```
As shown in the t-test, we can say with 95% confidence that the means are statistically significant. Hibernating animals tend to have smaller body sizes than non-hibernating animals. However, this once again does not take into account confounding influential variables. 

Overall, I have found that the authors' hypothesis was supported, although it is likely that my conclusions came at error. I have also seen that there is a relationship between body size and hibernation, as well as substrate use and hibernation. Further data analysis involving phylogenetic relationship accountability is needed.
